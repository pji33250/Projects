...
$(document).ready(function(){
        ...
        "columnDefs": [
            {
                ...
                "render": function ( data, type, row ) {
                    return '<a href="/library/search/booklist.php?category=subject&id=' + data + '">' + data + '</a>';
                }
            },
            {
                "targets": -1,
    ...
    {
        e.preventDefault();
        $('.lightbox_content h2').text('Add subject');
        $('#form_company button').text('Add subject');
    ...
    $(document).on('click', '.function_edit a', function(e){
        ...
        request.done(function(output){
            if (output.result == 'success') {
                $('.lightbox_content h2').text('Edit subject');
                $('#form_company button').text('Edit subject');
                $('#form_company').attr('class', 'form edit');
                $('#form_company').attr('data-id', myid);
                $('#form_company .field_container label.error').hide();
                $('#form_company .field_container').removeClass('valid').removeClass('error');
                $('#form_company #SUBJECT_NAME').val(output.data[0].NAME);
                hide_loading_message();
                show_lightbox();
            }
            else {
