...
{
    // Execute job
    if ($job == 'get_records') // Get/List all records
    {
        ...
        $allRecords = executeQuery("SELECT ID FROM SUBJECT");
        ...
        $sql = "SELECT ID, NAME FROM SUBJECT WHERE 1=1";
    ...
    elseif ($job == 'get_record') // Get record for editing
    {
        $myid = $_GET['id'];
        $rec = fetchFirstRecord('SUBJECT', 'ID', $myid);
        if ($rec) {
            $result  = 'success';
    ...
    {
        $myname = trim( mysqli_real_escape_string($GLOBALS['db'], $_GET['SUBJECT_NAME']) );
        if (empty($myname)) {
            $message = 'add_record() skipped: subject-name cannot be empty';
        }
        else {
            // Check if record exists, if not then add or skip it
            $rec = fetchFirstRecord('SUBJECT', 'NAME', $myname);
            if (!$rec) {
                // Add record
                $mytime = gmdate('Y-m-d H:i:s');
                $query = "INSERT INTO SUBJECT SET NAME='$myname', ADDED_DATE='$mytime'";
    ...
    elseif ($job == 'edit_record') // Edit an existing record
    {
        ...
        $myname = trim( mysqli_real_escape_string($GLOBALS['db'], $_GET['SUBJECT_NAME']) );
        if (empty($myname)) {
            $message = 'edit_record() skipped: subject-name cannot be empty';
        }
        else {
            // Check if record exists, if not then add or skip it
            $existingRecordQuery = "SELECT * FROM SUBJECT WHERE NAME = '$myname' AND ID <> $myid";
            $rec = fetchFirstRecordByQuery($existingRecordQuery);
            if (!$rec) {
                // update record
                $mytime = gmdate('Y-m-d H:i:s');
                $query = "UPDATE SUBJECT SET NAME='$myname', UPDATED_DATE='$mytime' WHERE ID=$myid";
                $queryRet = executeQuery($query);
                if ($queryRet) {
                    $result  = 'success';
    ...
    {
        $myid = $_GET['id'];
        $queryBookCheck = "SELECT ID FROM BOOK WHERE SUBJECT_ID=$myid";
        $rec = fetchFirstRecordByQuery($queryBookCheck);
        if (!$rec) {
            // Only if the record has not been referred, can be deleted from DB
            $query = "DELETE FROM SUBJECT WHERE ID=$myid";
            $queryRet = executeQuery($query);
            if ($queryRet) {
                $result  = 'success';
