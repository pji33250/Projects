...
$(document).ready(function(){
    ...
    {
        e.preventDefault();
        $('.lightbox_content h2').text('Add author');
        $('#form_company button').text('Add author');
    ...
    $(document).on('click', '.function_edit a', function(e){
        ...
        request.done(function(output){
            if (output.result == 'success'){
                $('.lightbox_content h2').text('Edit Author');
                $('#form_company button').text('Edit Author');
                $('#form_company').attr('class', 'form edit');
                $('#form_company').attr('data-id', myid);
                $('#form_company .field_container label.error').hide();
                $('#form_company .field_container').removeClass('valid').removeClass('error');
                $('#form_company #AUTHOR_NAME').val(output.data[0].NAME);
                hide_loading_message();
                show_lightbox();
            }
            else {
    ...
    $(document).on('click', '.function_delete a', function(e){
        ...
        {
            //double confirm for deleting
            if (confirm("Last Chance: Do you really want to delete '" + myname + "'?"))
            {
                show_loading_message();
                var request = $.ajax({
